...
abstract class ThymusClientCliCommandRunner<T extends ThymusClientCliCommandConfig>
    ...
    private ThymusVCClient createThymusClient(T config)
        ...
        Option<Map<ThymusEndpoint, VcpToken>> tokens =
                getVcpTokens(configuration);
        return config
                ...
                .getOrElse(Try.of(() -> {
                    ...
                    tokens.forEach(t -> builder.withBearerTokenProvider(uri -> t.get(uri).getIdToken()));
                ...
                .getOrElseThrow(t -> new CsCliRunnerException(
    ...
    private Option<Map<ThymusEndpoint, VcpToken>> getVcpTokens(
            Configuration configuration) throws CsCliLoginException {
        if (!VcpTokenStore.exists()) {
            ...
            return Option.some(
                    Maps.uniqueIndex(
                            ...
                            t -> getThymusEndpoint(t.getVcpBaseUrl(), configuration)));
        }
    }

    private ThymusEndpoint getThymusEndpoint(
            String vcpBaseUrl, Configuration configuration) {
        return Option.ofOptional(Arrays.stream(configuration.getProviders())
                        .filter(c -> c.getBaseUrl().toString().equals(vcpBaseUrl))
                        .findFirst()
                        .map(c ->
                                ThymusEndpoint.Builder()
                                        .withServiceUri(c.getThymusServiceUrl())
                                        .build()))
                .getOrElseThrow(() ->
